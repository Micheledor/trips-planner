...
test('sortResponse should sort trips by duration when sort_by is "duration"', () => {
  const trips = [
    { duration: 3, cost: 200 },
    { duration: 5, cost: 100 },
    { duration: 7, cost: 150 },
...
test('sortResponse should handle an empty trips array', () => {
  const trips = [];
  const sortedTrips = sortResponse(trips, { sort_by: 'cost' });
  assert.deepEqual(sortedTrips, []);
});

test('sortResponse should handle a single trip in the array', () => {
  const trips = [{ duration: 5, cost: 100 }];
  const sortedTrips = sortResponse(trips, { sort_by: 'cost' });
  assert.deepEqual(sortedTrips, [{ duration: 5, cost: 100 }]);
});

test('sortResponse should maintain stability for trips with the same cost', () => {
  const trips = [
    { duration: 3, cost: 200 },
    { duration: 5, cost: 200 },
    { duration: 7, cost: 150 },
...
test('sortResponse should maintain stability for trips with the same duration', () => {
  const trips = [
    { duration: 3, cost: 200 },
    { duration: 5, cost: 100 },
    { duration: 5, cost: 150 },
...
test('generateCacheKey should return correct cache key', () => {
  const query = {
    origin: 'ARN',
    destination: 'ATL',
    sort_by: 'cost',
...
test('generateCacheKey should work with pagination', () => {
  const query = {
    origin: 'ARN',
    destination: 'ATL',
    sort_by: 'cost',
...
test('generateCacheKey should work with additional query params', () => {
  const query = {
    origin: 'ARN',
    destination: 'ATL',
    sort_by: 'cost',
...
test('generateCacheKey should sort query params alphabetically after origin and destination', () => {
  const query = {
    origin: 'ARN',
    destination: 'ATL',
    sort_by: 'cost',
...
test('generateCacheKey should be consistent with the order of origin and destination', () => {
  const firstQuery = {
    origin: 'ARN',
    destination: 'ATL',
    sort_by: 'cost',
